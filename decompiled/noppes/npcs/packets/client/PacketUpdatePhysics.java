...
extends PacketBasic {
    ...
    public PacketUpdatePhysics(int id, PlayMessages.SpawnEntity pkt) {
        this.id = id;
        this.pkt = pkt;
    }
    ...
    public static PacketUpdatePhysics decode(FriendlyByteBuf buf) {
        return new PacketUpdatePhysics(buf.readInt(), PlayMessages.SpawnEntity.decode((FriendlyByteBuf)buf));
    }
    ...
    public void handle() {
        ...
        if (PhysicsHelper.Enabled) {
            PhysicsHelper.resetEntityPhysics((Level)w, this.id);
        }
    }
