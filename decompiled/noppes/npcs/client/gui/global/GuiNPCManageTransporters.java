...
implements IScrollData {
    ...
    public void buttonEvent(GuiButtonNop guibutton) {
        ...
        if (id == 0 && this.selectCategory) {
            NoppesUtil.openGUI((Player)this.player, new GuiNPCTransportCategoryEdit(this.npc, this, "", -1));
        }
        if (id == 1) {
            ...
            if (this.selectCategory) {
                NoppesUtil.openGUI((Player)this.player, new GuiNPCTransportCategoryEdit(this.npc, this, this.slot.getSelectedString(), this.data.get(this.slot.getSelectedString())));
            }
        }
        if (id == 4) {
            if (this.selectCategory) {
                this.close();
                NoppesUtil.openGUI((Player)this.player, new GuiNPCGlobalMainMenu(this.npc));
            } else {
                this.title = "";
