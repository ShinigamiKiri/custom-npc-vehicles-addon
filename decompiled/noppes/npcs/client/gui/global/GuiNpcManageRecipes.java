...
ITextfieldListener {
    ...
    private Map<String, ResourceLocation> data = new HashMap<String, ResourceLocation>();
    ...
    public void setGuiData(CompoundTag compound) {
        ...
        this.container.setRecipe(recipe, this.player.m_9236_().m_9598_());
    ...
    protected void m_7286_(GuiGraphics graphics, float f, int x, int y) {
        ...
        for (int i = 0; i < this.container.width; ++i) {
            for (int j = 0; j < this.container.width; ++j) {
                graphics.m_280218_(this.slot, this.guiLeft + i * 18 + 7, this.guiTop + j * 18 + 34, 0, 0, 18, 18);
            }
        }
        graphics.m_280218_(this.slot, this.guiLeft + 86, this.guiTop + 60, 0, 0, 18, 18);
    }

    @Override
    public void setData(Vector<String> list, Map<String, Integer> data) {
        String name = this.scroll.getSelected();
        this.data = new HashMap<String, ResourceLocation>();
        for (String s : data.keySet()) {
            String[] arr = s.split("\\|");
            this.data.put(arr[0], new ResourceLocation(arr[1]));
        }
        Vector<String> res = new Vector<String>();
        for (String s : list) {
            res.add(s.split("\\|")[0]);
        }
        this.scroll.setList(res);
    ...
    public void unFocused(GuiTextFieldNop guiNpcTextField) {
        ...
        if (!name.isEmpty() && !this.data.containsKey(name)) {
            ...
            this.data.put(this.container.recipe.name, new ResourceLocation("customnpcs", this.container.recipe.name));
            this.selected = name;
