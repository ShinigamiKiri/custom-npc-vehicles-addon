...
extends GuiNPCInterface2 {
    ...
    public void m_7856_() {
        ...
        for (int i = 0; i < this.scenes.scenes.size(); ++i) {
            ...
            this.addButton(new GuiButtonNop((IGuiInterface)this, 1 + i * 10, this.guiLeft + 120, y, 60, 20, new String[]{"gui.disabled", "gui.enabled"}, scene.enabled ? 1 : 0));
            ...
            if (CustomNpcs.SceneButtonsEnabled) {
                this.addButton(new GuiButtonNop((IGuiInterface)this, 4 + i * 10, this.guiLeft + 232, y, 60, 20, new String[]{"gui.none", ClientProxy.Scene1.m_90860_(), ClientProxy.Scene2.m_90860_(), ClientProxy.Scene3.m_90860_()}, scene.btn));
            }
            y += 22;
        }
        if (this.scenes.scenes.size() < 6) {
            this.addTextField(new GuiTextFieldNop(101, (Screen)this, this.guiLeft + 4, y + 10, 190, 20, "Scene" + (this.scenes.scenes.size() + 1)));
    ...
    public void buttonEvent(GuiButtonNop button) {
        if (button.id < 60) {
            ...
            if (button.id % 10 == 1) {
                boolean bl = scene.enabled = button.getValue() == 1;
            }
            if (button.id % 10 == 2) {
                this.scene = scene;
