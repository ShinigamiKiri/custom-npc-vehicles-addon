...
extends GuiNPCInterface2 {
    ...
    public void m_7856_() {
        ...
        this.addButton(new GuiButtonYesNo((IGuiInterface)this, 16, this.guiLeft + 175, this.guiTop + 152, !this.npc.advanced.orderedLines));
    }
    ...
    public void buttonEvent(GuiButtonNop guibutton) {
        int id = guibutton.id;
        if (id == 0) {
            NoppesUtil.openGUI((Player)this.player, new GuiNPCLinesEdit(this.npc, this.npc.advanced.worldLines));
        }
        if (id == 1) {
            NoppesUtil.openGUI((Player)this.player, new GuiNPCLinesEdit(this.npc, this.npc.advanced.attackLines));
        }
        if (id == 2) {
            NoppesUtil.openGUI((Player)this.player, new GuiNPCLinesEdit(this.npc, this.npc.advanced.interactLines));
        }
        if (id == 5) {
            NoppesUtil.openGUI((Player)this.player, new GuiNPCLinesEdit(this.npc, this.npc.advanced.killedLines));
        }
        if (id == 6) {
            NoppesUtil.openGUI((Player)this.player, new GuiNPCLinesEdit(this.npc, this.npc.advanced.killLines));
        }
        if (id == 7) {
            NoppesUtil.openGUI((Player)this.player, new GuiNPCLinesEdit(this.npc, this.npc.advanced.npcInteractLines));
        }
        if (id == 16) {
