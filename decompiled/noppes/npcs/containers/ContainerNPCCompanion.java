    ...
    public ContainerNPCCompanion(int containerId, Inventory playerInventory, int entityId) {
        ...
        for (int k = 0; k < 3; ++k) {
            for (int j1 = 0; j1 < 9; ++j1) {
                this.m_38897_(new Slot((Container)playerInventory, j1 + k * 9 + 9, 6 + j1 * 18, 87 + k * 18));
            }
        }
        for (int l = 0; l < 9; ++l) {
            this.m_38897_(new Slot((Container)playerInventory, l, 6 + l * 18, 145));
        }
        if (this.role.talents.containsKey((Object)EnumCompanionTalent.INVENTORY)) {
            ...
            for (int i = 0; i < size; ++i) {
                this.m_38897_(new Slot((Container)this.role.inventory, i, 114 + i % 3 * 18, 8 + i / 3 * 18));
            }
        }
        if (this.role.getTalentLevel(EnumCompanionTalent.ARMOR) > 0) {
            this.m_38897_(new SlotCompanionArmor(this.role, (Container)npc.inventory, 0, 6, 8, EquipmentSlot.HEAD));
            this.m_38897_(new SlotCompanionArmor(this.role, (Container)npc.inventory, 1, 6, 26, EquipmentSlot.CHEST));
            this.m_38897_(new SlotCompanionArmor(this.role, (Container)npc.inventory, 2, 6, 44, EquipmentSlot.LEGS));
            this.m_38897_(new SlotCompanionArmor(this.role, (Container)npc.inventory, 3, 6, 62, EquipmentSlot.FEET));
        }
        if (this.role.getTalentLevel(EnumCompanionTalent.SWORD) > 0) {
            this.m_38897_(new SlotCompanionWeapon(this.role, (Container)npc.inventory, 4, 79, 17));
        }
    }
